...
Picture* IHS(Picture* P_MUL, Picture* P_PAN){
	...
	long double tran1_tmp[3][3] = { 
		{ 1/(long double)3,         1/(long double)3,         1/(long double)3},      
        { -sqrt(2.0)/(long double)6,  -sqrt(2.0)/(long double)6,  sqrt(2.0)/(long double)3},      
		{ 1/sqrt(2.0),   -1/sqrt(2.0),  0}
	};

	long double tran2_tmp[3][3] = { 
		{ 1,  -1/sqrt(2.0),  1/sqrt(2.0)},      
        { 1,  -1/sqrt(2.0),  -1/sqrt(2.0)},     
        { 1,  sqrt(2.0),     0 }
	};
	matrix tran1(bandNum, bandNum);
	matrix tran2(bandNum, bandNum);
	for (int i = 0;i<bandNum;i++){
		for(int j = 0; j<bandNum ;j++){
			tran1.SetElem(i+1,j+1,tran1_tmp[i][j]);
			tran2.SetElem(i+1,j+1,tran2_tmp[i][j]);
		}	
	}

	long double **trans_in_tmp = new long double *[bandNum];
	for(int i = 0; i< bandNum; i++){
		trans_in_tmp[i] = new long double [Ylen*Xlen];
	}
	for(unsigned short k = 0 ;k<3 ;k++){
		for(int i = 0 ; i< Xlen; i++){
			for (int j = 0 ; j < Ylen ;j++){
				trans_in_tmp[k][j*Xlen+i] = (long double) (M_MUL[i][j] -> getBand(k)); // reshape() in matlab is col by col
			}
		}
	}
	matrix trans_in(bandNum, Xlen*Ylen, trans_in_tmp);
	delete []trans_in_tmp;
	
	/*
	for(int i = 0 ;i<3;i++){
		for(int j = 0;j<3;j++){
			cout<<trans_in.GetElem(i+1,j+1)<<" ";
		}
		cout<<endl;
	}	
	system("PAUSE");
	...
	cout<<"trans set done."<<endl;

	matrix trans_res = tran1 * trans_in;

	cout<<"RGB ==> IHS done."<<endl;
	
	//cout<<trans_res.GetCols()<<"	"<<trans_res.GetRows()<<endl;
	for(int i = 0 ; i< Xlen; i++){
		for (int j = 0 ; j < Ylen ;j++){
			trans_res.SetElem(1, j*Xlen+i+1, (long double) (M_PAN[i][j] -> getBand(0)) );// reshape() in matlab is col by col
		}
	}

	cout<<"I replacing done."<<endl;

	matrix trans_res_2 = tran2 * trans_res;

	cout<<"IHS ==> RGB done."<<endl;
	
	for(int i = 0 ; i< Xlen; i++){
		for (int j = 0 ; j < Ylen ;j++){
			for(unsigned short k = 0 ;k<3 ;k++){
				M_fus[i][j] ->set(k,(unsigned short)(trans_res_2.GetElem(k+1,j*Xlen+i+1))); // reshape() in matlab is col by col
				M_fus[i][j] ->cutOff();
			}
		}
	}
